...
- Changed: Dropped support for Ruby 3.2 (now requires Ruby >= 3.3). Ruby 3.2's `Zlib.gzip`/`Zlib.gunzip`/`Zlib.deflate`/`Zlib.inflate` module helpers grow their output buffer while the GVL is released during (de)compression, so a concurrent thread (e.g. another consumer decoding a message) can corrupt the buffer and raise `Zlib::DataError` ("invalid stored block lengths"). The upstream fix relies on safe no-GVL offload, a Ruby 3.3+ primitive (ruby/zlib#88, ruby/zlib#89).
