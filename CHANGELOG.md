...
- Added: `on_connect:` constructor option for `AMQP::Client`. Invoked with the client after each successful (re)connection, after consumer recovery. If it raises, the exception is logged and the supervisor continues. (#83)
- Added: `Client#queue("")` now declares a server-named (anonymous) queue and returns a `Queue` whose `#name` reflects the broker-assigned name. Pair with `on_connect:` to redeclare and rebind on every reconnect. (#89)
- Changed: Consumer auto-recovery on reconnect is best-effort — a consumer whose queue is gone (e.g. a server-named one from a previous connect) is dropped with a warn instead of crashing recovery.
