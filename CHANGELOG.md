...
- Fixed: The built-in `gzip` and `deflate` coders no longer use the `Zlib.gzip`/`Zlib.gunzip`/`Zlib.deflate`/`Zlib.inflate` module helpers, which share process-global state and are not thread-safe on Ruby < 3.3. Concurrent consumers decoding compressed messages could raise `Zlib::DataError` ("invalid stored block lengths"). The coders now use instance-based `Zlib::GzipWriter`/`GzipReader` and `Zlib::Deflate`/`Zlib::Inflate`.
