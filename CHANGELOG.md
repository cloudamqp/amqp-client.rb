...
- Fixed: The built-in `gzip` and `deflate` coders no longer use the `Zlib.gzip`/`Zlib.gunzip`/`Zlib.deflate`/`Zlib.inflate` module helpers. Those helpers grow their output buffer (`rb_str_set_len`) while the GVL is released during (de)compression, so a concurrent thread (e.g. another consumer decoding a message) can corrupt the buffer and raise `Zlib::DataError` ("invalid stored block lengths"). Observed only on Ruby 3.2, since the upstream fix relies on safe no-GVL offload, a Ruby 3.3+ primitive (ruby/zlib#88, ruby/zlib#89). The coders now use instance-based `Zlib::GzipWriter`/`GzipReader` and `Zlib::Deflate`/`Zlib::Inflate`.
